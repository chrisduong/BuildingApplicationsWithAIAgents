...
from frameworks.langgraph_agents.ecommerce_customer_support.customer_support_agent import (
    graph,
)
...
def test_refund_flow():
    ...
    assert any(
        "refund" in m.content.lower() for m in messages
    ), "Should confirm refund in user message"


def test_cancel_flow():
    order = {"order_id": "B54321", "status": "Processing", "total": 59.99}
    user_msg = [
        HumanMessage(content="Please cancel my order, I don't need it anymore.")
    ]
    ...
    assert any(
        "cancel" in m.content.lower() for m in messages
