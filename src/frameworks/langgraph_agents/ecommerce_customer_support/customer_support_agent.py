...
def update_address_for_order(order_id: str, shipping_address: dict) -> str:
    ...
    print(
        f"[TOOL] update_address_for_order(order_id={order_id}, address={shipping_address})"
    )
    log_to_loki(
        "tool.update_address_for_order",
        f"order_id={order_id}, address={shipping_address}",
    )
...
def call_model(state: AgentState):
    ...
    first: AIMessage = cast(AIMessage, llm.invoke(full))
    ...
    if getattr(first, "tool_calls", None):
        for tc in first.tool_calls:
            ...
            messages.append(ToolMessage(content=str(out), tool_call_id=tc["id"] or ""))
